                ...
                if (doc instanceof DeploymentTemplateDoc) {
                    for (const scope of doc.allScopes) {
                        ...
                        for (const info of infos) {
                            ...
                            let f = false;
                            if (info.parent) {
                                lenses.push(new ShowChildParentCodeLens(scope, info.resourceObject.span, (<IJsonResourceInfo>info.parent).resourceObject.span, `Child of ${(<IJsonResourceInfo>info.parent).getFriendlyName()}`));
                                f = true;
                            }
                            if (info.children.length > 0) {
                                ...
                                const children = info.children.map(child => (<IJsonResourceInfo>child).getFriendlyName()).join(", ");
